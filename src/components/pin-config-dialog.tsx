...
export function PinConfigDialog({ open, onOpenChange, onConfigSaved }: PinConfigDialogProps) {
  ...
  const [activeTab, setActiveTab] = useState<'setup' | 'manage' | 'permissions'>('setup');
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={`border-amber-900/60 bg-stone-950 text-amber-100 transition-all p-3 sm:p-6 max-h-[90vh] overflow-y-auto w-[96vw] max-w-full ${
        activeTab === 'permissions' ? 'sm:max-w-[780px]' : 'sm:max-w-[460px]'
      }`}>
        <DialogHeader>
        ...
        <div className="space-y-4 py-1 sm:py-2">
          <div className={`grid gap-1 rounded-md bg-stone-900 p-1 border border-stone-800 text-[11px] sm:text-xs font-medium ${
            configured ? 'grid-cols-3' : 'grid-cols-2'
          }`}>
            {configured && (
              <button
                type="button"
            ...
            <button
              type="button"
              onClick={() => { setActiveTab('permissions'); setErrorMsg(null); }}
              className={`py-1.5 px-1 sm:px-2 text-center rounded transition-colors truncate ${
                activeTab === 'permissions' ? 'bg-amber-600/30 text-amber-200 font-semibold shadow-sm' : 'text-stone-400 hover:text-amber-200'
              }`}
            >
              Schede Giocatore
            </button>
          ...
          {/* Modalità Permessi Schede (Matrice Permessi) */}
