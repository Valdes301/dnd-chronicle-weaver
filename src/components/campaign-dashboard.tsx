...
import { GranularQuestCreator } from './dnd/granular-quest-creator';
import { PartyXpManager } from './dnd/party-xp-manager';
import { Button } from './ui/button';
import { BookMarked, Loader2, Sparkles, Pencil, Check, Archive, ChevronLeft, Compass, Trophy, ScrollText } from 'lucide-react';
import { 
    AlertDialog, 
    AlertDialogAction, 
    AlertDialogCancel, 
...
export function CampaignDashboard({
  ...
  const [storyMode, setStoryMode] = useState<'timeline' | 'granular' | 'xp'>('timeline');
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-w-full overflow-x-hidden">
      ...
      {/* Selettore Modalità Storia / Creazione Granulare / Assegnazione PX */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 p-1.5 bg-muted/40 backdrop-blur-sm rounded-xl border border-border/50 max-w-2xl mx-auto shadow-sm w-full">
        <Button
          variant={storyMode === 'timeline' ? 'default' : 'ghost'}
          size="sm"
          onClick={() => setStoryMode('timeline')}
          className="gap-2 text-xs sm:text-sm font-medium w-full justify-center"
        >
          <ScrollText className="h-4 w-4 shrink-0" />
          Cronologia & Rapido
        </Button>
        <Button
          variant={storyMode === 'granular' ? 'default' : 'ghost'}
          size="sm"
          onClick={() => setStoryMode('granular')}
          className="gap-2 text-xs sm:text-sm font-medium w-full justify-center"
        >
          <Compass className="h-4 w-4 text-amber-400 shrink-0" />
          Crea Quest (Granulare)
        </Button>
        <Button
          variant={storyMode === 'xp' ? 'default' : 'ghost'}
          size="sm"
          onClick={() => setStoryMode('xp')}
          className="gap-2 text-xs sm:text-sm font-medium w-full justify-center"
        >
      ...
      {storyMode === 'granular' ? (
        <div className="pt-2">
          <GranularQuestCreator
            campaign={campaign}
            onBackToStories={() => setStoryMode('timeline')}
            onQuestSaved={(newSession) => {
              onConfirmSession(newSession);
              setStoryMode('timeline');
            }}
            onPcsUpdated={() => {
              router.refresh();
            }}
