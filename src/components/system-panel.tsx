...
export function SystemPanel({
  ...
  const [activeTab, setActiveTab] = useState<'control' | 'backgrounds' | 'settings' | 'homebrew' | 'model'>('control');
  ...
  return (
    <div className="space-y-6">
      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
        <TabsList className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 w-full h-auto bg-neutral-900/80 p-1.5 border border-border/40 rounded-xl mb-6 gap-1.5">
          <TabsTrigger 
        ...
        <TabsContent value="control" className="space-y-6 mt-0">
          ...
          <div className="space-y-3 pt-2">
            <div className="flex items-center gap-2 text-sm font-semibold text-amber-400 uppercase tracking-wider">
              <ShieldCheck className="h-4 w-4" /> Sicurezza & Sigilla Schermo
            </div>
              ...
              <Card className="bg-card/60 border-amber-500/20 hover:border-amber-500/40 transition-all">
                <CardHeader className="p-4 pb-2">
                  <CardTitle className="text-base flex items-center gap-2 text-amber-300">
                    ...
                    Impostazioni Sigilla Schermo
                  </CardTitle>
                  <CardDescription className="text-xs">
                    Configura PIN, password di recupero, timer di inattività e schede protette per la modalità giocatore.
                  </CardDescription>
                </CardHeader>
                <CardContent className="p-4 pt-2">
                  <Button 
