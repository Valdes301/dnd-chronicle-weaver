...
import React, { useState, useMemo, memo, useRef, useLayoutEffect, useCallback, useEffect } from 'react';
...
import { Download, Loader2, Wand, ChevronLeft, ChevronRight, Sword, Shield, Heart, Beaker, Zap, Trophy } from 'lucide-react';
...
const CardBackDisplay = memo(({ type }: { type: 'item' | 'spell' }) => {
    const bgUrl = type === 'spell' ? `/api/assets/card-back-magie.jpg?t=${Date.now()}` : `/api/assets/card-back-oggetti.jpg?t=${Date.now()}`;
    return (
        <div 
            style={{ 
                width: CARD_WIDTH, 
...
const CardDisplay = memo(({ originalName, card, onUpdate, isForPrinting = false }: { originalName: string, card: ItemCardData, onUpdate: (itemName: string, field: keyof ItemCardData, value: string) => void, isForPrinting?: boolean }) => {
    ...
    const getTechIcon = () => {
        ...
        const iconStyle = { width: '12px', height: '12px', display: 'block', marginTop: '3px' };
    ...
    // Mostriamo la box se c'è un valore tecnico O se è una ricompensa
    const showTechBox = (card.damage && card.techType !== 'none') || card.techType === 'reward';
        ...
        >
            ...
            <div className="relative z-10 flex flex-col h-full">
                ...
                >
                    ...
                    {showTechBox && (
                        <div 
                            className="bg-black/5 px-2 rounded-md border border-black/10 flex flex-col overflow-hidden"
                            style={{ 
                                height: '26px',
                                marginTop: '2px'
                            }}
                        >
                            <div style={{ display: 'flex', alignItems: 'center', justifyItems: 'center', height: '22px', gap: '5px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '14px', width: '12px' }}>
                                    {getTechIcon()}
                                </div>
                                <span 
                                    style={{ 
                                ...
                                >
                                    {card.damage || (card.techType === 'reward' ? 'Premio' : '')}
                                </span>
                            </div>
...
interface SelectedCard { name: string; type: 'item' | 'spell' };

export function ExperimentalCardGenerator({ allItems, dbSpells, initialSelection }: ExperimentalCardGeneratorProps) {
    const [selectedCards, setSelectedCards] = useState<SelectedCard[]>(initialSelection || []);
    ...
    const allSelectableItems = useMemo(() => {
        // Separazione degli oggetti per tab
        const items = [...allItems].filter(i => (i as any).techType !== 'reward').sort((a, b) => a.name.localeCompare(b.name));
        const rewards = [...allItems].filter(i => (i as any).techType === 'reward').sort((a, b) => a.name.localeCompare(b.name));
        const spells = [...dbSpells].sort((a, b) => a.name.localeCompare(b.name));
        return { items, rewards, spells };
    }, [allItems, dbSpells]);
    ...
    const filteredRewards = useMemo(() => {
        const lowerSearch = searchTerm.toLowerCase();
        return allSelectableItems.rewards.filter(item => item.name.toLowerCase().includes(lowerSearch));
    }, [searchTerm, allSelectableItems.rewards]);
    ...
    // Sincronizzazione selezione iniziale e popolazione cardData
    useEffect(() => {
        if (initialSelection && initialSelection.length > 0) {
            ...
            setCardData(prev => {
                ...
                initialSelection.forEach(sel => {
                    if (sel.type === 'item') {
                        const item = allItems.find(i => i.name === sel.name);
                        if (item) {
                        ...
                        const spell = dbSpells.find(s => s.name === sel.name);
                        if (spell) {
                            next[sel.name] = { name: spell.name, level: spell.level || '', casting_time: spell.casting_time || '', range: spell.range || '', components: spell.components || '', duration: spell.duration || '', school: spell.school || '', description: spell.description || '', type: 'magie' };
                        }
                    }
                });
                return next;
    ...
    }, [initialSelection, allItems, dbSpells]);
    ...
    const handleGenerateImages = async () => {
        ...
        toast({ title: `Generazione di ${totalPages} fogli A4 (Alta Risoluzione)...` });
                    ...
                    <CardContent>
                        <Tabs value={activeTab} onValueChange={setActiveTab}>
                            <TabsList className="grid w-full grid-cols-3">
                                ...
                                <TabsTrigger value="ricompense">Premi</TabsTrigger>
                            ...
                            <ScrollArea className="h-[65vh] mt-4">
                                ...
                                <TabsContent value="ricompense" className="space-y-2 pr-4">
                                    {filteredRewards.map(item => (
                                        <div key={item.name} className="flex items-center space-x-2 py-1 border-b border-border/50 last:border-0">
                                            <Checkbox id={`r-${item.name}`} checked={selectedCards.some(c => c.name === item.name)} onCheckedChange={(checked) => handleCardSelect(item.name, 'item', !!checked)} />
                                            <label htmlFor={`r-${item.name}`} className="text-sm font-medium flex-1 cursor-pointer truncate">{item.name}</label>
                                        </div>
                                    ))}
                                </TabsContent>
